...
function mockFs(t, isCommonConfigDirectory) {
  ...
  helper.isDirectory = function(p) {
    t.assert.strictEqual(p, path.join('appPath', 'common/config'));
  ...
  fs.readdirSync = function(p) {
    t.assert.strictEqual(p, 'appPath');
...
test('Loader constructor properly set paths', t => {
  ...
  t.assert.strictEqual(loader.appPath, 'appPath');
});
...
function testIsMultiModule(isCommonConfigDirectory, modules) {
  return t => {
    ...
    var loader = createLoader();
    t.assert.deepStrictEqual(loader.modules, modules);
  };
}
